...
from agents.column_actor import ColumnActor
from agents.column_critic import ColumnCritic
...
# SAC
# Actor
column_actor = ColumnActor(input_dim=4, hidden_dim=8).to(device)

# Critics
column_critic_1 = ColumnCritic(input_dim=4, action_dim=3, hidden_dim=8).to(device)
column_critic_2 = ColumnCritic(input_dim=4, action_dim=3, hidden_dim=8).to(device)

# Target Critics
target_column_critic_1 = copy.deepcopy(column_critic_1).to(device)
target_column_critic_2 = copy.deepcopy(column_critic_2).to(device)

# Set to train mode
column_actor.train()
column_critic_1.train()
column_critic_2.train()
...
for ep in tqdm(range(epochs)):
    ...
    while not done and n_step < max_steps:
        ...
        if action_id == 1:  # -> column
            ...
            # Sample action
            action, log_prob = column_actor.sample(in_comp)  # action: [pair_idx, recov_lk, recov_hk]
            pair_idx = int(action[0])
            recov_lk = float(action[1])
            recov_hk = float(action[2])

            if np.random.rand() < epsilon[ep]:
                pair_idx = np.random.randint(low=0, high=6)

            lk, hk = column_actor.index_to_pair(pair_idx)
            
            #print(lk, hk, recov_lk, recov_hk)
